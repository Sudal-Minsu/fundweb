...
- config_Jin.py에서 보안/계좌/DB 설정을 import
...
# 🔐 같은 디렉토리에 config_Jin.py가 있어야 함 (DB_CONFIG, get_api_keys, ACCOUNT_INFO 제공)
try:
    from config_Jin import DB_CONFIG, get_api_keys, ACCOUNT_INFO
except Exception as e:
    print("❌ config_Jin.py 불러오기 실패. 같은 폴더에 있는지, 함수/변수명이 맞는지 확인하세요.")
    print(f"오류: {e}")
    sys.exit(1)
...
RULE2_OUTPUT_DIR = "rule_2_결과"  # 요약 지표 저장
os.makedirs(RULE2_OUTPUT_DIR, exist_ok=True)

TOTAL_RISK_BUDGET_ALL = 5_000_000_00   # 전체 리스크 예산
MAX_BUY_BUDGET = 10_000_000            # 종목당 최대 매수 예산

# 매매 로그 파일 (lsmc_결과/trade_log.csv)
...
def issue_access_token():
    ...
    if not app_key or not app_secret:
        print("❌ app_key/app_secret이 비어 있습니다. config_Jin.py의 keyring 설정을 확인하세요.")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    BUYLIST_DIRS = [RULE2_OUTPUT_DIR, OUTPUT_DIR]
    buy_list_path = None
