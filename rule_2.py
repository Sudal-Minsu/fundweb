...
TEST_PERIOD_DAYS = 50    # 백테스트 기간 (일 단위)
RSI_n = 2                # RSI 계산에 사용할 기간 (n일)
STOCK_NUMBER = 20        # 종목 수 (최대 200개)

# 백테스트 시작 날짜 (사용자가 원하는 날짜로 수정 가능)
import pandas as pd
BACKTEST_START_DATE = pd.to_datetime("2024-10-07")          
...
# 매매 조건 (예측 확률 기준)
BUY_PROB_THRESHOLD = 0.7          # 매수 신호 확률 기준 (상승 예측: class 0)
SELL_PROB_THRESHOLD = 0.7         # 매도 신호 확률 기준 (하락 예측: class 1)
...
def prepare_sequences(features_2d, close_prices, future_n, target_percent):
    sequences, targets = [], []
    for i in range(1, len(features_2d) - SEQ_LEN - future_n + 1):
        window = features_2d[i-1:i-1+SEQ_LEN]
        base_price = close_prices[i-1+SEQ_LEN - 2]
        future_price = close_prices[i-1+SEQ_LEN - 1 + future_n]
...
def prepare_stock_models(engine, top_codes):
    ...
    for code in top_codes:
        ...
        df_stock['Volatility_N'] = df_stock['Close'].rolling(FUTURE_N+1).std()
        df_stock['Volatility_spike'] = (df_stock['Volatility_N'] > df_stock['Volatility_N'].rolling(30).mean()).astype(int)
        df_stock['Volume_jump'] = (df_stock['Volume'] > df_stock['Volume'].rolling(20).mean() * 1.5).astype(int)
        
        # 종가 관련
        df_stock['Momentum_N'] = df_stock['Close'] / df_stock['Close'].shift(FUTURE_N+1) - 1
...
""" 백테스트 함수 """
def run_custom_backtest(stock_models, global_test_dates):
    ...
    for test_date in tqdm(global_test_dates, desc="날짜별 전략 백테스트"):
        ...
        # 투자금 설정
        day_max_invest = max(total_cash * 0.1, INITIAL_CAPITAL * 0.1)
        ...
        for code in code_list:
            ...
            if shares > 0 and buy_price is not None:
                if close_yesterday > buy_price * 1.05:
                    ...
                    data["sell_reason"] = "익절"
                elif close_yesterday <= buy_price * 0.97:  
                    data["sell_candidate"] = 100
                    data["sell_reason"] = "손절"
                elif close_yesterday > buy_price and pred_prob[1] > SELL_PROB_THRESHOLD and rsi_yesterday > 55:
                    data["sell_candidate"] = 100
                    data["sell_reason"] = "예측 기반 매도"
                elif close_yesterday > buy_price and rsi_yesterday > 60:
                    data["sell_candidate"] = 100
                    data["sell_reason"] = "RSI > 60"
                    
            # 매수 조건 
            if ma20 and ma60 and ma20 > ma60 and (rsi_yesterday < 45) and (pred_prob[0] > BUY_PROB_THRESHOLD):
                ...
                if sell_percent > 0:
                    ...
                    if qty_to_sell > 0:
                        ...
                        if data["shares"] == 0 and data.get("buy_price") is not None:
                            profit_pct = (close_yesterday / data["buy_price"]) - 1
    ...
    for code, data in stock_models.items():
        if data.get("shares", 0) > 0 and data.get("buy_price") is not None:
            ...
            profit_pct = (final_price / data["buy_price"]) - 1
            trade_success = True if profit_pct >= TARGET_PERCENT else False
...
""" 백테스트 결과 시각화 """
def run_custom_backtest_wrapper(stock_models, global_test_dates):
    total_portfolio = run_custom_backtest(stock_models, global_test_dates)
...
""" 평균 성능 CSV 저장 """
def save_average_score_csv_from_memory(stock_models, loss_curve_dict, filename="성능 지표.csv"):
    records = []
    for code, data in stock_models.items():
...
""" 거래 성공률 CSV 저장 """
def save_trade_success_rate(stock_models, filename="거래 성공률.csv"):
    records = []
    for code, data in stock_models.items():
...
""" 매매 로그 CSV 저장 """
def save_trade_log_csv(stock_models, filename="매매 로그.csv"):
    all_records = []
    for code, data in stock_models.items():
...
""" 상위/하위 손실 곡선 시각화 """
def plot_top_bottom_loss_curves(loss_curve_dict, top_n=10):
    loss_summary = []
...
""" SHAP 분석 함수 """
def run_shap_analysis(stock_models):
    ...
    selected_trades = pd.concat([top3, bottom3], ignore_index=True)

    if selected_trades.empty:
        print("[SHAP] 선택된 매매 기록이 없습니다.")
        return

    # SHAP heatmap 그리기용 subplot 준비
    num_plots = len(selected_trades)
    nrows = 2 if num_plots > 3 else 1
...
def main():
    ...
    run_custom_backtest_wrapper(stock_models, global_test_dates)
    save_average_score_csv_from_memory(stock_models, loss_curve_dict)
    save_trade_success_rate(stock_models)
    save_trade_log_csv(stock_models)   
    plot_top_bottom_loss_curves(loss_curve_dict, top_n=10)
    run_shap_analysis(stock_models)
