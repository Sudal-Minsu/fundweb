...
# 오늘 날짜 및 시작 날짜 설정 (최근 3년치)
today = datetime.date.today()
start_date = today - datetime.timedelta(days=365 * 3)  # 최근 3년으로 변경
...
def get_top_200_codes():
    ...
    for code in tqdm(top_codes_pool, desc="조건 필터링 중"):
        try:
            ...
            # 데이터가 200일 이상 있어야 통과(3년치 데이터는 보통 750일 이상)
            if len(df_stock) >= 200:
                # 최근 100일간 데이터 따로 추출
                recent_100 = df_stock.tail(100)
                # 최근 100일 중 거래량이 비어있거나 1 이하인 경우 제외
...
def process_stock(code):
    try:
        ...
        # _: 튜플에서 인덱스 무시(데이터만 사용), iterrows(): dataframe의 각 행을 돌며 튜플 반환
        for _, row in df.iterrows():
            ...
            cursor.execute("""
                INSERT INTO stock_data (Date, Code, Close, Volume)
                VALUES (%s, %s, %s, %s)
...
def run_stock_data():
    ...
    # 3년 이전 데이터 삭제
    cutoff_date = today - datetime.timedelta(days=365 * 3)
